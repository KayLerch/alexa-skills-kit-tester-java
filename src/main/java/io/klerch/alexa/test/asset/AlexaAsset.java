...
public enum AlexaAsset implements AlexaAssetValidator {
    Card {
        @Override
        public boolean exists(final SpeechletResponseEnvelope response) {
            return AlexaAssertion.HasCard.isTrue(response);
        }

        @Override
        public boolean equals(final SpeechletResponseEnvelope response, final Object value) {
        ...
        @Override
        public boolean matches(final SpeechletResponseEnvelope response, final String pattern) {
            final ObjectMapper mapper = new ObjectMapper();
            try {
                final String json = mapper.writeValueAsString(response.getResponse().getCard());
    ...
    DirectivePlayAudioItemStreamOffset {
        @Override
        public boolean exists(final SpeechletResponseEnvelope response) {
            // long cannot be null so it always ifExists - somehow
            return DirectivePlayAudioItemStream.exists(response);
        }
    ...
    private static <TDirective extends Directive> Optional<TDirective> getDirectiveOfType(final SpeechletResponseEnvelope response, final Class<TDirective> directiveClass) {
